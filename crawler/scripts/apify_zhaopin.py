...
# zhaopin-scraper covers 51job/zhaopin - PAY_PER_EVENT
ZHAOPIN_ACTOR = 'zhaopin-scraper'
...
def run_apify_zhaopin(
    ...
    if not force_paid:
        log.warning('zhaopin-scraper is PAY_PER_EVENT (paid). Use --force-paid to run.')
        return {'total': 0, 'inserted': 0, 'failed': 0, 'error': 'paid_actor_skipped'}

    keywords = keywords or ['python', 'java', '算法', '前端', '大模型']
    log.info('Calling Actor: %s | keywords=%s | max=%d', ZHAOPIN_ACTOR, keywords, max_items)

    client = ApifyClient(token)
    run_input = {'searchKeywords': keywords, 'maxResults': max_items}

    try:
        run = client.actor(ZHAOPIN_ACTOR).call(run_input=run_input)
    ...
    log_request(source_site='zhaopin', target_url=f'apify://actor/{ZHAOPIN_ACTOR}/dataset/{dataset_id}',
    ...
    for item in items:
        try:
            title = _extract_str(item.get('jobName') or item.get('jobTitle') or item.get('title'))
            company = _extract_str(item.get('companyName') or item.get('company') or item.get('companyShortName'))
            description = item.get('jobDesc') or item.get('jobDescription') or item.get('description') or ''
            url = item.get('jobUrl') or item.get('url') or ''
            city = _extract_str(item.get('workCity') or item.get('city') or item.get('location'))
            education = _extract_str(item.get('education') or item.get('eduLevel'))
            salary = item.get('salary') or ''
            item_id = item.get('positionId') or item.get('id') or item.get('itemId') or ''

            if not title and not description:
                skipped += 1; continue

            if not url or not url.strip():
                url = f'apify://zhaopin/{dataset_id}/{item_id}' if item_id else f'apify://zhaopin/{dataset_id}/title/{hash(title) % 10**8}'

            robots_ok = is_allowed(url, _COMPLIANCE_UA)
            log_request(source_site='zhaopin', target_url=url, robots_allowed=robots_ok,
    ...
    log.info('Zhaopin done: %s', summary)
    return summary


def main():
    parser = argparse.ArgumentParser(description='Apify Zhaopin (51job/智联) crawler')
    ...
    summary = run_apify_zhaopin(max_items=args.max, keywords=args.keywords, dry_run=args.dry_run, force_paid=args.force_paid)
