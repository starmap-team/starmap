...
def run_integration(limit: int, backend_url: str, skip_existing: bool = True) -> dict:
    ...
    for i, item in enumerate(pending, 1):
        ...
        if "error" in result and "position_name" not in result:
            log.error("抽取失败: %s", result["error"])
            mark_jd_extracted(item["id"], success=False, error=result["error"])
            failed += 1
            errors.append({"jd_raw_id": item["id"], "error": result["error"]})
            continue
        ...
        r = extraction_dao.upsert_extraction(
            jd_content=item["clean_text"][:50000],  # 限长 50K（API 限制）
