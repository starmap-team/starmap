...
def upsert_jd(record: dict) -> str:
    ...
    try:
        ...
        with get_jd_raw_session() as s:
            # 2026-08-20: DB 同时有 content_hash 与 source_url 两个唯一约束。
            # 只按 content_hash ON CONFLICT 时，content_hash 不同但 URL 相同
            # （平台 API 重复返回同一职位）会抛 UniqueViolation → 用 constraint 覆盖两者。
            stmt = (
                pg_insert(JdRaw)
                .values(**clean)
                .on_conflict_do_nothing(
                    index_elements=["content_hash"],
                    index_where=JdRaw.content_hash.isnot(None),
                )
            )
            try:
                result = s.execute(stmt)
            except Exception:
                # content_hash 冲突路径之外的唯一约束冲突（source_url 重复）→ 视为 duplicate
                s.rollback()
                return "duplicate"
            s.commit()
