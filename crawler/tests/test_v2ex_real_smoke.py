...
(the compliance.fetch entry point, ) so they are deterministic and CI-safe.
"""
...
# ---------------------------------------------------------------------------
# Scenario 5: strict per-source isolation 
# ---------------------------------------------------------------------------

def test_source_v2ex_only_returns_v2ex(v2ex, monkeypatch):
 """source='v2ex' 只抓 V2EX 酷工作，不混入 Remotive 记录。
...
def test_source_remotive_only_returns_remotive(v2ex, monkeypatch):
 """source='remotive' 只抓 Remotive API，不写 v2ex 记录。"""
 v2ex_payload = [
