...
const totalPositions = computed(() => {
  ...
  return ip !== null && ip !== undefined ? ip : graphStore.domains.reduce((s: number, d) => s + d.position_count, 0)
})
        ...
        <main class="graph-main">
          ...
          >
            ...
            <GraphToolbar
              ...
              @zoom-in="viewMode === '2d' ? graph2DRef?.zoomBy(1.2) : graph3DRef?.zoomIn()"
              @zoom-out="viewMode === '2d' ? graph2DRef?.zoomBy(0.8) : graph3DRef?.zoomOut()"
              @zoom-fit="viewMode === '2d' ? graph2DRef?.fitView() : graph3DRef?.fitView()"
              @toggle-layout="toggleLayout()"
              @reset-highlight="resetHighlight()"
              @max-nodes-change="onMaxNodesChange"
              @proficiency-filter="onProficiencyFilter"
              @camera-preset="onCameraPreset($event)"
