...
export function renderDomainLayer(deps: DomainLayerDeps): void {
  ...
  const entranceNodes = graphNodes.map((n: NodeData) => ({ id: n.id, style: { fillOpacity: 0, scale: 0.3 } }))
...
export function renderPositionLayer(deps: PositionLayerDeps): void {
  ...
  const shouldAnimateGrowth = deps.showEvolution() && limitedPositions.length > 0
  ...
  for (const p of limitedPositions) {
    ...
    const nodeOpacity = shouldAnimateGrowth ? 0 : 0.85
    const nodeScale = shouldAnimateGrowth ? 0.3 : 1
  ...
  graphInstance.render()
  deps.animateNodeGrowth?.(shouldAnimateGrowth ? limitedPositions.map(p => p.id) : [])
...
export function renderDetailLayer(deps: DetailLayerDeps): void {
  ...
  graphNodes.push({ id: posId, style: { size: 50, fill: NODE_TYPE_COLORS.Position, fillOpacity: 0.9, stroke: cv('--primary-hover'), lineWidth: 3, labelText: posNode ? displayName(posNode.properties) : '岗位', labelFill: cv('--primary-foreground'), labelFontSize: 13, labelFontWeight: 'bold' as const, labelPlacement: 'center' as const, shadowColor: 'rgba(59,130,246,0.3)', shadowBlur: 12 } })
  ...
  for (const e of posEdges) {
    ...
    graphNodes.push({ id: e.target_id, style: { size, fill: skillColor, fillOpacity: 0.8, stroke: isRequired ? cv('--success') : cv('--warning'), lineWidth: 1, labelText: skillNode.properties.name, labelFill: cv('--foreground'), labelFontSize: 10, labelPlacement: 'bottom' as const, labelOffsetY: 4 } })
