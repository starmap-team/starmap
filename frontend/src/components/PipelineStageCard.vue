...
const STAGE_DESCRIPTIONS: Record<string, { role: string; deps: string[]; status_meaning: Record<string, string> }> = {
  crawl: {
    role: '从启用的数据源采集原始岗位信息，写入原始岗位数据表',
    deps: [],
    status_meaning: {
      pending: '等待前置依赖（无前置）',
  ...
  dedup: {
    role: '内容相似度比对去重，标记重复记录',
    deps: ['crawl'],
    status_meaning: {
      pending: '等待 crawl 完成',
      running: '内容相似度模糊比对',
      completed: '去重完成（唯一 + 重复计数已报告）',
      failed: '去重失败 — 检查缓存服务是否可用',
      skipped: '上游失败导致跳过',
      cancelled: '用户取消',
    },
  },
  ...
  import: {
    ...
    status_meaning: {
      ...
      failed: '抽取失败 — 可重试或检查调用配额',
      skipped: '上游失败导致跳过',
      cancelled: '用户取消',
    },
  },
  graph_sync: {
    role: '将主数据同步到图谱（按顺序分步同步，保证数据一致）',
    deps: ['import'],
    status_meaning: {
      pending: '等待 import 完成',
      running: '正在同步到图谱（分步写入 + 可选核对）',
      completed: '图谱同步完成',
      failed: '图谱写入失败 — 已记录待重试',
      skipped: '上游失败导致跳过',
      cancelled: '用户取消',
    },
  },
...
const STAGE_METRIC_MEANING: Record<string, (s: { records_processed?: number; records_seen?: number; records_new?: number | null; records_duplicate?: number | null }) => string> = {
  ...
  import: (s) => `本阶段用智能模型抽取 ${s.records_processed ?? 0} 条记录的岗位/技能；若为 0 表示上游没有新记录需要抽取`,
