...
async def _sync_neo4j_on_audit(neo4j_driver: Any, item_type: str, item_name: str, status: str) -> None:
 ...
 修复: 用 MERGE 而非 MATCH+SET，确保节点不存在时自动创建。
...
async def sync_all_positions_to_neo4j(
 ...
 `canonical_id = str(PositionRecord.id)`，与 `_sync_neo4j_on_audit` 一致（ canonical_id 复用）。

 单条失败不影响其余记录：错误收集进 `failed` 列表后继续（沿 M3 仅观察不阻断）。
...
async def approve_audit(
 ...
 fix: when a new SkillRecord / PositionRecord is created
 from an approved ReviewQueue row, set review_status='approved' so
 the new review-workflow column stays consistent. Previously
...
async def batch_audit(
 ...
 Same as approve_audit — newly-created SkillRecord /
 PositionRecord get review_status='approved' so the column
 stays consistent.
 """
 result = await session.execute(
