...
async def approve_audit(item_id: int, session: AsyncSession, neo4j_driver: Any | None = None) -> AuditItem:
    ...
    if row.entity_type in _SKILL_ENTITY_TYPES:
        ...
        if existing.scalar_one_or_none() is None:
            session.add(SkillRecord(name=row.entity_name, category="general", source_count=1))
