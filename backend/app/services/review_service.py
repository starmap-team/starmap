...
async def count_by_status(session: AsyncSession) -> dict[str, int]:
 ...
 admin overview KPI "待审演化 " was silently displaying the
 `skill_pending_review` count (skills) instead of the real
 evolution queue count, which made the evolution workflow look
