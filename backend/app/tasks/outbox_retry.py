"""Outbox retry worker — 自动重放失败的 graph_write_outbox 行（Phase 23 Task 1）。

DC-02 / DF-01: 采集写入不可退——`run_batch_extract_jd` / `execute_graph_sync` 写 Neo4j
...
#: 每行最大重试次数（达上限 → 告警日志 + audit_events，DF-01 暴露）
MAX_RETRY_COUNT = 3
...
async def _list_retryable_outbox(
    session: Any, batch_size: int = BATCH_SIZE,
) -> list[Any]:
...
async def _replay_outbox_row(session_factory: Any, row: Any, driver: Any) -> bool:
    ...
    （DF-01 暴露）。单行失败不阻断 batch（D-06 fail-soft）。
    """
    from sqlalchemy import select
...
async def _alert_max_retry(session_factory: Any, row: Any, error: str) -> None:
    """retry_count 达上限 → 告警日志 + audit_events 原始 SQL INSERT（DF-01 暴露）。
