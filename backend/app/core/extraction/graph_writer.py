...
Spec : 7 node types — Position, Skill, KnowledgeArea, Tool, Certificate, LearningResource, Industry
Spec : 8 relationship types — REQUIRES, PREREQUISITE, EVOLVES_TO, USES, BELONGS_TO, CERTIFIES, RECOMMENDED_FOR, APPLIES_TO
"""
...
async def merge_position(driver: Any, position_data: dict[str, Any], canonical_id: str | None = None) -> dict[str, Any]:
 ...
 canonical_id: PG PositionRecord.id — **MERGE 键**（）。PG 是
 ...
 if not canonical_id:
 # (checkpoint:decision): MERGE 键从 name 切为 canonical_id。
...
async def merge_skill(driver: Any, skill_name: str, metadata: dict[str, Any] | None = None, canonical_id: str | None = None) -> dict[str, Any]:
 ...
 canonical_id: PG SkillRecord.id — **MERGE 键**（）。name 降级
 为普通 SET 属性；缺失时 raise ``GraphProjectionError`` 拒绝产生孤儿。

 Notes:
 ``source_count`` 口径（, IC-06）：SET 使用 **max 语义**
 ...
 if not canonical_id:
 # (checkpoint:decision): 同 merge_position——无 canonical_id
 ...
 *,
 ...
 """Create a REQUIRES relationship between Position and Skill .

 (DC-05): REQUIRES 边属性契约唯一真值 —— 本函数与演化投影
...
async def write_extraction_to_graph(
 ...
 """Write a full extraction result to Neo4j (/).
 ...
 if not _raw_name or not str(_raw_name).strip:
 # -03 (Fix B3): 缺失 position_name 静默跳过, 不阻塞 batch
 logger.warning(
 ...
 # : REQUIRES 端点按 canonical_id MATCH（写路径主键收敛）
...
async def batch_write_extractions(
 ...
 # -03 (Fix B4): try/except 单条隔离, 一条失败不阻塞整个 batch
 try:
