...
def get_prompt(name: str, **kwargs: Any) -> str:
    ...
    Uses ``string.Template`` with ``$placeholder`` syntax so that
    literal JSON braces (``{}``) in the templates do **not** need
    escaping.
    ...
    """
    template_str = _PROMPT_REGISTRY.get(name)
    ...
    filled = Template(template_str).safe_substitute(**kwargs)
