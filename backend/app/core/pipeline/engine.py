...
executor.py 保留兼容重导出（），存量调用方零改动；新代码请直接
...
async def advance_pipeline(run_id: uuid.UUID) -> None:
 ...
 STOP flag 检查 — if Redis flag `pipeline:stop:{run_id}` is set,
 ...
 # fix (2026-08-15): records_new=0 是合法值（全部重复），
 ...
 await session.commit # explicit commit to persist running state immediately
