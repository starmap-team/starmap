"""Pipeline import 阶段（ + + Task 5）。

LLM 技能抽取 + PG 持久化。按 发 3 子步骤事件：extract / normalize / persist。
本模块从 executor.execute_import 迁出；executor.py 保留兼容重导出（）。
拆分：`_update_source_after_import` 辅助随阶段迁入本模块。
"""
