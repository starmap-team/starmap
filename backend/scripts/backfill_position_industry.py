...
from app.core.extraction.industry import UNCLASSIFIED_INDUSTRY_LITERAL, is_generic_industry
...
async def collect_candidates(session_maker, limit: int, include_pending: bool = False) -> list[PositionRecord]:
    """扫描「未分类」字面量的岗位（多层防御 Phase 1 之后的语义）。

    默认 `include_pending=False` —— 仅扫 approved 已发布岗位（影响公开图谱）。
    设置 `include_pending=True` 时同时扫 pending_review —— 用于清理 dashboard
    显示的「未分类海洋」，但 pending 后续会被 admin 审核覆盖，所以谨慎使用。

    Phase 1A (040 migration) 把所有 NULL/'' 归一化为「未分类」字面量后，
    真正"缺失 industry"的语义 =「未分类」字面量。本脚本现在扫这种"语义缺失"行。
    """
    async with session_maker() as session:
        if include_pending:
            status_filter = PositionRecord.review_status.in_(["approved", "pending_review"])
        else:
            status_filter = (PositionRecord.review_status == "approved")
        rows = (
...
async def _write_industry(session_maker, pos: PositionRecord, industry_zh: str) -> bool:
    """把 industry 写回 DB（仅在行仍「未分类」时写入，防 TOCTOU）。

    Phase 1 (2026-08-17): 040 迁移把所有 NULL/'' 归一化为「未分类」字面量后，
    backfill 的语义 = 找到「未分类」行 → LLM 分类 → 写回 canonical 桶。
    如果行已经被人工审核改成其他值（如 admin reclassify），不覆盖。
    """
    async with session_maker() as s:
        row = (
            await s.execute(select(PositionRecord).where(PositionRecord.id == pos.id))
        ).scalar_one_or_none()
        if row is not None and (row.industry is None
                                 or row.industry == ""
                                 or row.industry == UNCLASSIFIED_INDUSTRY_LITERAL):
