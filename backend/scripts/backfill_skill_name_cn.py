...
对 skill_records 中 name_cn 为空的英文技能用 LLM 翻译补全。
用法:
    cd backend
    poetry run python -m scripts.backfill_skill_name_cn [--limit N] [--dry-run]
"""
...
async def backfill(limit: int, dry_run: bool) -> None:
    ...
    print(f"[backfill-skills] {len(rows)} 个技能缺中文名" + ("（dry-run）" if dry_run else ""))
    ...
    print(f"[backfill-skills] 完成 {done}/{len(rows)}")
    await engine.dispose()


def main() -> None:
    parser = argparse.ArgumentParser(description="批量回填技能中文名")
