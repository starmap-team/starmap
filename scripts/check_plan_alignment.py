...
校验三类可机械验证的事实，输出偏差清单，防止计划与代码再漂移：
 1. Golden Set 规模 vs 计划 目标（JD>=100 / 简历>=50 / 匹配>=100）
 2. OpenAPI 契约端点完整性（实现已存在、契约必须收录的关键端点）
 3. Alembic 迁移链规模（报告口径，漂移预警）

用法:
 python scripts/check_plan_alignment.py # 报告模式，恒 exit 0
 python scripts/check_plan_alignment.py --strict # 任一失败 exit 1（CI 门禁用） 原型只做机械事实比对；语义级偏差（公式族/策略）由附录E 裁决表人工维护。
"""
...
def check_contract -> list[tuple[bool, str]]:
 ...
 ok = p in paths
 results.append((ok, f"契约端点 {p}: {'已收录' if ok else '缺失（ 漂移）'}"))
 return results
...
def main -> int:
 ...
 sections = {
 "Golden Set 规模 vs 计划 ": check_golden_sets,
